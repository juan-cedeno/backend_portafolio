"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const express_1 = __importDefault(require("express"));
const cors_1 = __importDefault(require("cors"));
const dotenv_1 = __importDefault(require("dotenv"));
const coursesRouter_1 = __importDefault(require("./routers/coursesRouter"));
const dbConecction_1 = require("./db/dbConecction");
const skillRouter_1 = __importDefault(require("./routers/skillRouter"));
const proyectController_1 = __importDefault(require("./routers/proyectController"));
dotenv_1.default.config();
const app = (0, express_1.default)();
app.use((0, cors_1.default)());
(0, dbConecction_1.dbConecction)();
app.use(express_1.default.json());
app.use('/api/courses', coursesRouter_1.default);
app.use('/api/skill', skillRouter_1.default);
app.use('/api/proyect', proyectController_1.default);
app.listen(process.env.PORT, () => {
    console.log(process.env.PORT);
});
